                ...
                if (CGFloat(index) < (CGFloat(dataProvider.chartXMax) * animator.phaseX) / CGFloat(setCount))
                {
					if let e = set.entryForXIndex(index) as! BarChartDataEntry!
					{
						...
						if (drawHighlightArrowEnabled)
						{
							...
							let d = sqrt(pixelToValueMatrix.b * pixelToValueMatrix.b + pixelToValueMatrix.d * pixelToValueMatrix.d)
							let n = sqrt(pixelToValueMatrix.a * pixelToValueMatrix.a + pixelToValueMatrix.c * pixelToValueMatrix.c)
							let xToYRel = abs( d / n )
